...
export const Dashboard: React.FC = () => {
  ...
  if (!data || !Array.isArray(contracts) || contracts.length === 0) return <div className="p-8">Loading dashboard...</div>;

  const totalHours = contracts.reduce((acc, c) => acc + (parseFloat(c.totalHours) || 0), 0);
  const totalUsed = contracts.reduce((acc, c) => acc + (parseFloat(c.usedHours) || 0), 0);
